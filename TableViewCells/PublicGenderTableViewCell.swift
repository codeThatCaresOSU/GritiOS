//
//  PublicGenderTableViewCell.swift
//  SettingsPage
//
...
class PublicGenderTableViewCell: UITableViewCell {
    ...
    @objc func switchChanged(sender: UISwitch!){
        ...
        if let user = Auth.auth().currentUser {
            //sender.isOn ? FirebaseVars.ref.child("Users").child(user.uid).child("Settings").child("genderVisible").setValue(true) : FirebaseVars.ref.child("Users").child(user.uid).child("Settings").child("genderVisible").setValue(false)
        }
    }
    ...
    func createNotificationSwitch() -> UISwitch {
        ...
        if let user = Auth.auth().currentUser {
            /**FirebaseVars.ref.child("Users").child(user.uid).child("Settings").child("genderVisible").observe(.value, with: { snapshot  in
