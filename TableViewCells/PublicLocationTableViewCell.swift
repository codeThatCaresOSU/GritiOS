//
//  PublicLocationTableViewCell.swift
//  SettingsPage
//
...
class PublicLocationTableViewCell: UITableViewCell {
    ...
    @objc func switchChanged(sender: UISwitch!){
        ...
        if let user = Auth.auth().currentUser {
            //sender.isOn ? FirebaseVars.ref.child("Users").child(user.uid).child("Settings").child("locationVisible").setValue(true) : FirebaseVars.ref.child("Users").child(user.uid).child("Settings").child("locationVisible").setValue(false)
        }
    }
    ...
    func createNotificationSwitch() -> UISwitch {
        ...
        if let user = Auth.auth().currentUser {
//            FirebaseVars.ref.child("Users").child(user.uid).child("Settings").child("locationVisible").observe(.value, with: { snapshot  in
